...
public class FileArchives extends AbstractConnector implements Connector {
	...
	public void connect(MessageContext messageContext) throws ConnectException {
		...
		String suffixs = getParameter(messageContext, "suffixs") == null ? ""
				: getParameter(messageContext, "suffixs").toString();
		String newFileName = getParameter(messageContext, "archivefilename") == null ? ""
				: getParameter(messageContext, "archivefilename").toString();

		String archiveType = getParameter(messageContext, "archivetype") == null ? ""
				: getParameter(messageContext, "archivetype").toString();

		String filepattern = getParameter(messageContext, "filepattern") == null ? ""
				: getParameter(messageContext, "filepattern").toString();
		String dirpattern = getParameter(messageContext, "dirpattern") == null ? ""
				: getParameter(messageContext, "dirpattern").toString();
		boolean archivedirectory = getParameter(messageContext,
				"archivedirectory") == null ? false : Boolean
				.parseBoolean(getParameter(messageContext, "archivedirectory")
						.toString());
		...
		if (suffixs.equals("")) {
			if (archivedirectory) {
				...
				fileList = FileUtils.listFiles(inputDirectory,
		...
		if (filepattern.equals("") && dirpattern.equals("")) {
			...
			for (File filterFile : fileList) {
				if (new FilePattenMatcher(FILE_PATTERN).validate(filterFile
						.getName())) {
					filteredList.add(filterFile);
				} else if (filterFile.isDirectory()
						&& new FilePattenMatcher(DIR_PATTERN)
								.validate(filterFile.getName())) {
					filteredList.add(filterFile);
				}
			}
		}
		try {
			if (archiveType.equals(ArchiveType.TAR_GZIP.toString())) {
				new FileCompressUtil().compressFiles(filteredList, file,
