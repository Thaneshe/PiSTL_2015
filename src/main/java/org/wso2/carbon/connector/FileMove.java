...
public class FileMove extends AbstractConnector implements Connector {
	...
	public void connect(MessageContext messageContext) throws ConnectException {
		...
		String filebeforepprocess = getParameter(messageContext,
				"filebeforeprocess") == null ? "" : getParameter(
				messageContext, "filebeforeprocess").toString();

		String fileafterprocess = getParameter(messageContext,
				"fileafterprocess") == null ? "" : getParameter(messageContext,
		...
		try {
			resultStatus = moveFile(fileLocation, filename, filebeforepprocess,
					fileafterprocess, newFileLocation);
	 ...
	 * @return
	 */
	private boolean moveFile(String fileLocation, String filename,
			String filebeforepprocess, String fileafterprocess,
			String newFileLocation) throws FileSystemException {
		...
		FileObject remoteFile = manager.resolveFile(fileLocation + filename,
				opts);
		FileObject newFile = manager.resolveFile(newFileLocation + filename,
				opts);
		...
		if (!fileafterprocess.equals("")) {
			FileObject fAfterProcess = manager.resolveFile(fileafterprocess
					+ filename, opts);
