...
public class ResultPayloadCreater {
	...
	private static final Log log = LogFactory
			.getLog(ResultPayloadCreater.class);
	 ...
	 */
	public OMElement performSearchMessages(String output)
			throws XMLStreamException,
	...
	public static boolean buildFile(FileObject file, MessageContext msgCtx,
			String contentType, String streaming) throws SynapseException {
		...
		try {
			if (contentType == null || contentType.trim().equals("")) {
				...
				} else if (file.getName().getExtension().toLowerCase()
						.endsWith("txt")) {
					contentType = "text/plain";
				}
			} else {
				// Extract the charset encoding from the configured content type
				// and
				...
				try {
					if (contentType != null) {
						charSetEnc = new ContentType(contentType)
			...
			if (log.isDebugEnabled()) {
				log.debug("Processed file : " + file + " of Content-type : "
						+ contentType);
			}
			...
			if (builder instanceof DataSourceMessageBuilder
				...
				in = null;
				dataSource = ManagedDataSourceFactory
						.create(new FileObjectDataSource(file, contentType));
			} else {
				in = new AutoCloseInputStream(file.getContent()
						.getInputStream());
				dataSource = null;
			}
			...
			// We need this to build the complete message before closing the
			// stream
			...
			throw new SynapseException(
					"Error while processing the file/folder", e);
		} finally {
			if (dataSource != null) {
				dataSource.destroy();
			}
