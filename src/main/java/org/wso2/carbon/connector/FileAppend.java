...
public class FileAppend extends AbstractConnector implements Connector {
	...
	public void connect(MessageContext messageContext) throws ConnectException {
		...
		String filename = getParameter(messageContext, "file") == null ? ""
				: getParameter(messageContext, "file").toString();
		String content = getParameter(messageContext, "content") == null ? ""
				: getParameter(messageContext, "content").toString();
		String ftpFileLocation = getParameter(messageContext, "ftpfilelocation") == null ? ""
				: getParameter(messageContext, "ftpfilelocation").toString();

		String filebeforepprocess = getParameter(messageContext,
				"filebeforepprocess") == null ? "" : getParameter(
				messageContext, "filebeforepprocess").toString();
		String fileafterprocsess = getParameter(messageContext,
				"fileafterprocsess") == null ? "" : getParameter(
				messageContext, "fileafterprocsess").toString();
		String encoding = getParameter(messageContext, "encoding") == null ? ""
				: getParameter(messageContext, "encoding").toString();
		int offset = getParameter(messageContext, "offset") == null ? 0
				: Integer.parseInt(getParameter(messageContext, "offset")
						.toString());
		...
		try {
			resultStatus = appendFile(fileLocation, filename, content,
					encoding, filebeforepprocess, fileafterprocsess, offset);
		} catch (IOException e) {
	 ...
	 * @param resultStatus
	 */
	private void generateResult(MessageContext messageContext,
			boolean resultStatus) {
		ResultPayloadCreater resultPayload = new ResultPayloadCreater();
		String responce = "<result><success>" + resultStatus
				+ "</success></result>";
		OMElement element;
		try {
	 ...
	 */
	private boolean appendFile(String fileLocation, String filename,
			String content, String encoding, String filebeforepprocess,
		...
		if (!filebeforepprocess.equals("")) {
			FileObject fBeforeProcess = manager.resolveFile(filebeforepprocess
					+ filename, opts);
		...
		if (!fileafterprocsess.equals("")) {
			FileObject fAfterProcess = manager.resolveFile(fileafterprocsess
					+ filename, opts);
